...
pkill 'stratisd.sh|make|cargo|libstratis'
for j in $(ps aux | grep target/debug/stratisd\ --sim | grep -v grep | awk '{print $2}'); do
...
for j in $(dmsetup ls | grep thinpool-pool | awk '{print $1}'); do
...
for j in $(grep stratis_testing /proc/mounts | awk '{print $2}'); do
	echo $j
	umount $j
done
echo
...
for i in thin-fs thinpool-pool flex-thinmeta flex-thindata flex-mdv physical-originsub stratis_test_ stratis-.*private-.*-crypt; do
	for j in $(dmsetup ls | grep $i | awk '{print $1}'); do
...
for testdevs in $(grep /dev /etc/stratis/test_config.json | tr -d \"\,); do
	for wipetgt in $(blkid -p $testdevs | grep stratis | awk '{print $1}' | tr -d \:); do
